...
\begin{titlepage}
    ...
    \vspace{0.45cm}
    {\LARGE Grammar Design, Conflict Resolution, and Parsing Analysis \par}
    \vspace{0.6cm}
    ...
    \begin{tabular}{rl}
        ...
        \textbf{Assignment:} & Lab Assignment 3 \\
...
Direct browser link for the deployed matrix:
\href{https://sharduljunagade.github.io/compilers-cylite/cylite_A3/part4/parsing_table_matrix.html}{\texttt{parsing\_table\_matrix.html}}
...
\textbf{Note:} Additional implementation and analysis details were also included within earlier parts of this report wherever they were most relevant to the discussion (for example, directly alongside conflict explanations, derivation design, parsing-table generation, and diagnostics architecture) and the appendix at the end is also a part of additional information. This section consolidates supplementary points that are helpful as a standalone summary for Part 6.
...
\subsection{Implementation Details Summary}

The implementation was carried out as a staged workflow across the assignment parts. First, we extended the lexer and parser to support CYLite features such as \texttt{elif}, \texttt{pass}, \texttt{try-except}, \texttt{print}, and range-style iteration forms. Next, we resolved parser conflicts by combining precedence directives with targeted grammar refactoring, including safer placement of semantic actions for counters. After stabilizing the grammar, we added reverse-derivation extraction , automated ACTION/GOTO table generation from \texttt{yapl.output}, and structured line-column error diagnostics with expected-token hints . Together, these steps produced a parser that is both feature-complete for CYLite and easier to validate, debug, and extend.

\subsection{Analysis Highlights}

One key outcome was turning parser development into a reproducible workflow rather than a one-time debugging exercise. After each grammar update, we regenerated \\ \texttt{yapl.output}, re-checked conflict states, and validated behavior with targeted test programs that reflected the exact ambiguity being fixed. For analysis, we used both structural evidence (state transitions, reduction paths, precedence decisions) and behavioral evidence (before/after parser runs, semantic counters, and diagnostics output). This made it easier to justify why a fix was correct, not only because Bison warnings were reduced, but because parser behavior became deterministic and aligned with CYLite language specification.
