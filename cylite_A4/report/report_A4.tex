...
\begin{itemize}
    ...
    \item \textbf{Technical impact:} The grammar now emits entry, body, increment, and exit labels for loops and emits true/false branch structure for conditionals, including if-elif-else chains. In the latest update, \texttt{goto} and \texttt{LABEL} targets are emitted in the \texttt{result} field, keeping control-flow quadruples schema-consistent with readable output.
...
\begin{itemize}
    \item \textbf{Implementation details:} Added \texttt{print\_quad\_table()} and updated \texttt{main()} to print source, diagnostics, and generated TAC. The current implementation uses \texttt{format\_quad\_statement()} to derive a readable \texttt{statement} column from \texttt{(op, arg1, arg2, result)}.
    ...
    \item \textbf{Technical impact:} The output order now clearly mirrors the compilation pipeline: input program $\rightarrow$ parser success and counters $\rightarrow$ TAC table $\rightarrow$ reverse derivation. The generated table remains aligned for both expression and control-flow rows.
...
After normalizing control-flow emission, the generated IR now shows branch targets in a consistent way. For example, in control-flow test outputs, rows appear as \texttt{goto L2} and \texttt{LABEL L1} with the label stored in the \texttt{result} column, which keeps output formatting and interpretation unambiguous.
...
\section{Additional Worked Example: \texttt{tc3.c}}

To further explain intermediate-code generation, this section analyzes testcase \texttt{testcases\_a4/tc3.c}, which contains a simple \texttt{if} condition without \texttt{else}.
...
\subsection{Observed Generated IR}
\begin{center}
\small
\ttfamily
\begin{tabular}{|l|l|l|l|l|}
...
b=10 & = & 10 &  & b \\
t1=a<b & < & a & b & t1 \\
ifFalse t1 goto L1 & ifFalse & t1 &  & L1 \\
t2=a+1 & + & a & 1 & t2 \\
a=t2 & = & t2 &  & a \\
LABEL L1 & LABEL &  &  & L1 \\
\hline
...
\begin{enumerate}
    \item \textbf{Initialization:} The first two rows assign constants to \texttt{a} and \texttt{b}.
    \item \textbf{Condition evaluation:} \texttt{t1 = a < b} computes the relational condition and stores the Boolean-style result in a temporary.
    \item \textbf{Conditional branch:} \texttt{ifFalse t1 goto L1} skips the \texttt{if} body when the condition is false.
    \item \textbf{If-body execution:} When the condition is true, \texttt{t2 = a + 1} and then \texttt{a = t2} implement the update.
    \item \textbf{Join point:} \texttt{LABEL L1} marks the control-flow merge point after the \texttt{if} block.
\end{enumerate}

\subsection{Result}
This testcase confirms correct translation of a single-branch \texttt{if} statement into TAC: condition computation, conditional jump, body instructions, and final label placement are all emitted in proper execution order.
...
\subsection{Batch Result on Regenerated Testcases}
Following the latest parser updates, all testcase files \texttt{tc1.c} through \texttt{tc10.c} were rerun and outputs were regenerated in \texttt{testcases\_a4/tc1.txt} to \texttt{testcases\_a4/tc10.txt}. All runs completed successfully and produced parsing-success diagnostics along with IR tables.
